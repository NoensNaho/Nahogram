...
from kivy.uix.videoplayer import VideoPlayer  # NAHOOLS VİDEO MOTORU EKLENDİ!
...
# NAHOOLS VE AKIŞ İÇİN ORTAK KART TASARIMI
<PostCard>:
    orientation: "vertical"
    padding: "15dp"
    spacing: "10dp"
    ...
    MDBoxLayout:
        ...
        # VİDEO İSE BU ÇALIŞIR (NAHOOLS ÖZEL)
        VideoPlayer:
        ...
        MDBottomNavigation:
            ...
            MDBottomNavigationItem:
                ...
                MDBoxLayout:
                    ...
                    # YENİ PAYLAŞIM BUTONLARI (AKIŞ VE NAHOOLS AYRIMI)
                    MDBoxLayout:
                        adaptive_height: True
                        spacing: "15dp"
                        pos_hint: {"center_x": .5}
                        ...
                        MDRaisedButton:
                            ...
                            md_bg_color: 0.8, 0.2, 0.5, 1 # Nahools için farklı havalı bir renk (Mor/Pembe)
...
class PostCard(MDCard):
    ...
    db_hedef = StringProperty("gonderiler")  # 'gonderiler' veya 'nahools' olduğunu bilecek
...
class NahogramApp(MDApp):
    ...
    def __init__(self, **kwargs):
        ...
        # VİDEO UZANTILARI EKLENDİ!
        self.file_manager = MDFileManager(
            exit_manager=self.dosya_seciciyi_kapat,
            select_path=self.dosya_secildi,
            ext=['.png', '.jpg', '.jpeg', '.mp4', '.avi', '.mov', '.PNG', '.JPG', '.JPEG', '.MP4']
        )

    def build(self):
        self.theme_cls.theme_style = "Dark"
        self.theme_cls.primary_palette = "DeepPurple"
        if os.path.exists("ngram.jpeg"):
    ...
    def dosya_secildi(self, secilen_yol):
        ...
        if self.resim_hedefi == "post":
            ...
            # Eğer video ise önizleme resmine dokunma, fotoğraf ise göster
            is_video = secilen_yol.lower().endswith(('.mp4', '.avi', '.mov', '.mkv'))
            if not is_video:
                screen_add_ids.onizleme_resim.source = secilen_yol
            else:
                screen_add_ids.onizleme_resim.source = "ngram.jpeg"  # Video önizlemesi için varsayılan logo koydum

            screen_add_ids.btn_iptal.opacity = 1
            screen_add_ids.btn_iptal.disabled = False
            screen_add_ids.lbl_paylasim_uyari.text = ""  # Uyarıları temizle
    ...
    def giris_yap(self, kullanici_adi, sifre):
        ...
        def islem():
            try:
                res = requests.get(f"{FIREBASE_URL}/nahogram/kullanicilar/{kullanici_adi}.json")
            ...
            def islem():
                try:
                    ...
                    if kontrol.json() is None:
                        ...
                        requests.patch(f"{FIREBASE_URL}/nahogram/kullanicilar.json", json=yeni_hesap)
    ...
    def gonderi_begen(self, post_id, db_hedef):
        # db_hedef 'gonderiler' veya 'nahools' olacak, böylece nereyi güncelleyeceğini bilir
        islem_tipi = ""

        # Arayüzü bul ve anında kalbi kırmızı yap (Akış veya Nahools sekmesinde)
        ...
        def islem():
            if islem_tipi == "sil":
                ...
                requests.patch(f"{FIREBASE_URL}/nahogram/{db_hedef}/{post_id}/begenenler.json",
                               json={self.aktif_kullanici: True})
    ...
    def yorum_gonder(self, post_id, metin, db_hedef):
        ...
        for child in hedef_kutu.children:
            if hasattr(child, 'post_id') and child.post_id == post_id:
                child.comment_count += 1
                break

        def islem():
            res = requests.get(f"{FIREBASE_URL}/nahogram/{db_hedef}/{post_id}/yorumlar.json")
            mevcut_yorumlar = []
            if res.status_code == 200 and res.json():
                gelen = res.json()
    ...
    def akis_yenile(self):
        ...
        def islem():
            try:
                gonderiler_res = requests.get(f"{FIREBASE_URL}/nahogram/gonderiler.json")
    ...
    def nahools_yenile(self):
        ...
        def islem():
            try:
                nahools_res = requests.get(f"{FIREBASE_URL}/nahogram/nahools.json")
                if nahools_res.status_code == 200 and nahools_res.json():
                    ...
                    for key in reversed(post_keys):
                        ...
                        c_count = len(yorumlar) if isinstance(yorumlar, dict) else 0

                        cizilecekler.append({
                            "post_id": str(key),
                            "db_hedef": "nahools",  # NAHOOLS OLDUĞUNU BELİRTİYORUZ Kİ SİLME/BEĞENME DOĞRU YERE GİTSİN
                            "username": f"@{sahibi}",
                            "content": gonderi.get('icerik', ''),
                            "profil_foto": gonderi.get('profil_foto', ''),
                            "post_resim": gonderi.get("resim", ""),
    ...
    # --- İŞTE O DEVASA GÜMRÜK (PAYLAŞIM FİLTRESİ VE YÖNLENDİRME) ---
        ...
        def islem():
            try:
                ...
                if self.gecici_post_resmi:
                    if is_video:
                        # VİDEOYU YEREL OLARAK KOPYALIYORUZ (ÇÜNKÜ IMGBB VİDEO ALMAZ)
                        hedef_klasor = "nahogram_videolar"
                        os.makedirs(hedef_klasor, exist_ok=True)
                        yeni_yol = f"{hedef_klasor}/{os.path.basename(self.gecici_post_resmi)}"
                        shutil.copy(self.gecici_post_resmi, yeni_yol)
                        son_resim_url = yeni_yol
                    else:
                        # FOTOĞRAF İSE BULUTA FIRLAT (IMGBB)
                ...
                requests.post(f"{FIREBASE_URL}/nahogram/{db_odasi}.json", json=yeni_gonderi)
                requests.patch(f"{FIREBASE_URL}/nahogram/kullanicilar/{self.aktif_kullanici}.json",
                               json={"gonderi": guncel_gonderi_sayisi})
    ...
    def gonderi_sil(self, post_id, db_hedef):
        def islem():
            requests.delete(f"{FIREBASE_URL}/nahogram/{db_hedef}/{post_id}.json")
            benim_verim = requests.get(f"{FIREBASE_URL}/nahogram/kullanicilar/{self.aktif_kullanici}.json").json()
            yeni_gonderi_sayisi = max(0, benim_verim.get("gonderi", 1) - 1)
            requests.patch(f"{FIREBASE_URL}/nahogram/kullanicilar/{self.aktif_kullanici}.json",
                           json={"gonderi": yeni_gonderi_sayisi})
    ...
    # Diğer fonksiyonlar (Arama, Takip vb. dokunulmadı)
    def kullanici_ara(self, sorgu):
        ...
        def islem():
            res = requests.get(f"{FIREBASE_URL}/nahogram/kullanicilar.json")
            if res.status_code == 200 and res.json():
                ...
                liste_elemanlari = []
                benim_bilgilerim = requests.get(
                    f"{FIREBASE_URL}/nahogram/kullanicilar/{self.aktif_kullanici}.json").json()
                ...
                def arayuzu_ciz(dt):
                    ...
                    for veri in liste_elemanlari:
                        ...
                        l_elemani = TwoLineAvatarIconListItem(text=f"@{veri['k_adi']}",
                                                              secondary_text=f"{veri['takipci']} Takipçi")
                        if veri["p_foto"]:
    ...
    def takip_et(self, hedef_kisi, buton_objesi):
        def islem():
            benim_verim = requests.get(f"{FIREBASE_URL}/nahogram/kullanicilar/{self.aktif_kullanici}.json").json()
            hedef_veri = requests.get(f"{FIREBASE_URL}/nahogram/kullanicilar/{hedef_kisi}.json").json()
            ...
            requests.patch(f"{FIREBASE_URL}/nahogram/kullanicilar/{self.aktif_kullanici}.json",
                           json={"takip_edilenler": takip_listesi, "takip": yeni_takip_sayim})
            requests.patch(f"{FIREBASE_URL}/nahogram/kullanicilar/{hedef_kisi}.json",
                           json={"takipci": yeni_hedef_takipci})
    ...
    def profil_guncelle(self, bio_yazisi, hedef_yazisi):
        ...
        def islem():
            requests.patch(f"{FIREBASE_URL}/nahogram/kullanicilar/{self.aktif_kullanici}.json",
                           json={"bio": bio_yazisi, "hedef": hedef_yazisi})
    ...
    def mesaj_listesini_olustur(self):
        ...
        def islem():
            try:
                ben = requests.get(f"{FIREBASE_URL}/nahogram/kullanicilar/{self.aktif_kullanici}.json").json()
                if not ben: return
                takip_edilenler = ben.get("takip_edilenler", [])

                if not takip_edilenler:
                    def bos_ekran(dt):
                        liste.add_widget(TwoLineAvatarListItem(text="Sohbet Edecek Kimse Yok",
                                                               secondary_text="Önce birilerini takip etmelisin!"))

                    Clock.schedule_once(bos_ekran)
                    return

                ham_kullanicilar = requests.get(f"{FIREBASE_URL}/nahogram/kullanicilar.json").json()
                kullanicilar = {}
                if isinstance(ham_kullanicilar, dict):
                    kullanicilar = ham_kullanicilar
                elif isinstance(ham_kullanicilar, list):
                ...
                def arayuzu_ciz(dt):
                    for v in arkadaslar_verisi:
                        item = TwoLineAvatarListItem(text=f"@{v['k_adi']}", secondary_text="Sohbet başlat...")
    ...
    def sohbet_yenile(self, dt):
        ...
        def islem():
            res = requests.get(f"{FIREBASE_URL}/nahogram/sohbetler/{oda_id}.json")
            if res.status_code == 200 and res.json():
                gelen_veri = res.json()
                mesaj_listesi = []
