# ==================================================
# 1. KIVY AYARLARI VE MAKSİMUM GÜVENLİK KATI (SSL)
# ==================================================
import os
import sys
import certifi

# Tüm dış bağlantıları şifreli tünele zorlar
    ...
    MDBoxLayout:
        ...
        MDBottomNavigation:
            ...
            MDBottomNavigationItem:
                ...
                text: 'Ara'
                icon: 'magnify'
                on_tab_press: top_bar.title = "Arama"
                MDBoxLayout:
                    orientation: "vertical"
                    padding: "20dp"
                    spacing: "15dp"
            ...
            MDBottomNavigationItem:
                ...
                on_tab_press: top_bar.title = "Gönderi Paylaş"
                MDBoxLayout:
                    orientation: "vertical"
                    padding: "20dp"
                    spacing: "15dp"
...
class YorumDialogIcerik(MDBoxLayout): pass
...
class NahogramApp(MDApp):
    ...
    def __init__(self, **kwargs):
        ...
        self.is_fetching_akis = False
        self.is_fetching_nahools = False
        self.is_fetching_mesajlar = False

        # OTOMATİK GİRİŞ KONTROLÜ
        self.store = JsonStore("nahogram_kimlik.json")
    ...
    def on_start(self):
        ...
        if self.store.exists("oturum"):
            kayitli_kadi = self.store.get("oturum")["kadi"]
            kayitli_sifre = self.store.get("oturum")["sifre"]
            self.giris_yap(kayitli_kadi, kayitli_sifre)
    ...
    def resmi_buluta_yukle(self, yol):
        try:
            with open(yol, "rb") as file:
                ...
                files = {"image": file}
                res = requests.post("https://api.imgbb.com/1/upload", data=payload, files=files, timeout=15,
    ...
    def internet_ve_bulut_kontrol(self):
        ...
        def islem():
            try:
                cevap = requests.get(f"{FIREBASE_URL}/nahogram.json", timeout=10, verify=certifi.where())
                ...
                if bulut_linki:
                    try:
                        requests.patch(f"{FIREBASE_URL}/nahogram/kullanicilar/{self.aktif_kullanici}.json",
                                       json={"profil_foto": bulut_linki}, timeout=10, verify=certifi.where())
    ...
    def giris_yap(self, kullanici_adi, sifre):
        ...
        def islem():
            try:
                res = requests.get(f"{FIREBASE_URL}/nahogram/kullanicilar/{kullanici_adi}.json", timeout=10,
                                   verify=certifi.where())
                if res.status_code == 200 and res.json():
                    kullanici_verisi = res.json()
                    if kullanici_verisi["sifre"] == sifre:
                        self.store.put("oturum", kadi=kullanici_adi, sifre=sifre)
    ...
    def kayit_ol(self, kullanici_adi, sifre):
        ...
        if kullanici_adi and sifre:
            def islem():
                try:
                    kontrol = requests.get(f"{FIREBASE_URL}/nahogram/kullanicilar/{kullanici_adi}.json", timeout=10,
                                           verify=certifi.where())
                    if kontrol.json() is None:
                        ...
                        requests.put(f"{FIREBASE_URL}/nahogram/kullanicilar/{kullanici_adi}.json",
                                     json=yeni_hesap_bilgileri, timeout=10, verify=certifi.where())
    ...
    def gonderi_begen(self, post_id, db_hedef):
        ...
        def islem():
            try:
                if islem_tipi == "sil":
                    requests.delete(
                        f"{FIREBASE_URL}/nahogram/{db_hedef}/{post_id}/begenenler/{self.aktif_kullanici}.json",
                    ...
                    requests.patch(f"{FIREBASE_URL}/nahogram/{db_hedef}/{post_id}/begenenler.json",
                                   json={self.aktif_kullanici: True}, timeout=5, verify=certifi.where())
            except:
                pass
                ...
                def arayuzu_ciz(dt):
                    ...
                    self.yorum_dialog = MDDialog(
                        ...
                        buttons=[
                            ...
                            MDRaisedButton(text="Gönder", on_release=lambda x: self.yorum_gonder(post_id,
    ...
    def yorum_gonder(self, post_id, metin, db_hedef):
        ...
        for child in hedef_kutu.children:
            if hasattr(child, 'post_id') and child.post_id == post_id:
                def sayaci_artir(dt, c=child): c.comment_count += 1
    ...
    def goruntulenme_artir(self, post_id, db_hedef):
        def islem():
            try:
                requests.patch(f"{FIREBASE_URL}/nahogram/{db_hedef}/{post_id}/goruntuleyenler.json",
                               json={self.aktif_kullanici: True}, timeout=5, verify=certifi.where())
            except:
                pass
    ...
    def akis_yenile(self):
        if self.is_fetching_akis: return
        self.is_fetching_akis = True

        def islem():
            try:
                gonderiler_res = requests.get(f"{FIREBASE_URL}/nahogram/gonderiler.json", timeout=15,
                                              verify=certifi.where())
                if gonderiler_res.status_code == 200 and gonderiler_res.json():
                    ...
                    cizilecekler = []
                    post_keys = list(gonderiler.keys())[-20:]
                    ...
                    for key in reversed(post_keys):
                        ...
                        goruntuleyenler = gonderi.get('goruntuleyenler', {})
                        ...
                        v_count = len(goruntuleyenler) if isinstance(goruntuleyenler, dict) else len(
                            [g for g in goruntuleyenler if g])

                        if isinstance(goruntuleyenler, dict) and self.aktif_kullanici not in goruntuleyenler:
                            self.goruntulenme_artir(str(key), "gonderiler")
                            v_count += 1
    ...
    def nahools_yenile(self):
        if self.is_fetching_nahools: return
        self.is_fetching_nahools = True

        def islem():
            try:
                nahools_res = requests.get(f"{FIREBASE_URL}/nahogram/nahools.json", timeout=15, verify=certifi.where())
                if nahools_res.status_code == 200 and nahools_res.json():
                    ...
                    for key in reversed(post_keys):
                        ...
                        if isinstance(goruntuleyenler, dict) and self.aktif_kullanici not in goruntuleyenler:
                            self.goruntulenme_artir(str(key), "nahools")
                            v_count += 1
    ...
    def gonderi_paylas(self, icerik, hedef_tab):
        ...
        def islem():
            try:
                ...
                requests.post(f"{FIREBASE_URL}/nahogram/{db_odasi}.json", json=yeni_gonderi, timeout=20,
                              verify=certifi.where())
                requests.patch(f"{FIREBASE_URL}/nahogram/kullanicilar/{self.aktif_kullanici}.json",
                               json={"gonderi": guncel_gonderi_sayisi}, timeout=10, verify=certifi.where())
    ...
    def gonderi_sil(self, post_id, db_hedef):
        def islem():
            try:
                requests.delete(f"{FIREBASE_URL}/nahogram/{db_hedef}/{post_id}.json", timeout=10,
                                verify=certifi.where())
                benim_verim = requests.get(f"{FIREBASE_URL}/nahogram/kullanicilar/{self.aktif_kullanici}.json",
                                           timeout=10, verify=certifi.where()).json()
                yeni_gonderi_sayisi = max(0, benim_verim.get("gonderi", 1) - 1)
                requests.patch(f"{FIREBASE_URL}/nahogram/kullanicilar/{self.aktif_kullanici}.json",
                               json={"gonderi": yeni_gonderi_sayisi}, timeout=10, verify=certifi.where())
    ...
    def kullanici_ara(self, sorgu):
        ...
        def islem():
            try:
                res = requests.get(f"{FIREBASE_URL}/nahogram/kullanicilar.json", timeout=15, verify=certifi.where())
                if res.status_code == 200 and res.json():
                    ...
                    def arayuzu_ciz(dt):
                        ...
                        for veri in liste_elemanlari:
                            ...
                            btn = IconRightWidget(icon=ikon, theme_text_color=renk,
    ...
    def takip_et(self, hedef_kisi, buton_objesi):
        def islem():
            try:
                benim_verim = requests.get(f"{FIREBASE_URL}/nahogram/kullanicilar/{self.aktif_kullanici}.json",
                                           timeout=10, verify=certifi.where()).json()
                hedef_veri = requests.get(f"{FIREBASE_URL}/nahogram/kullanicilar/{hedef_kisi}.json", timeout=10,
                ...
                requests.patch(f"{FIREBASE_URL}/nahogram/kullanicilar/{self.aktif_kullanici}.json",
                               json={"takip_edilenler": takip_listesi, "takip": yeni_takip_sayim}, timeout=10,
                               verify=certifi.where())
                requests.patch(f"{FIREBASE_URL}/nahogram/kullanicilar/{hedef_kisi}.json",
                               json={"takipci": yeni_hedef_takipci}, timeout=10, verify=certifi.where())
    ...
    def profil_bilgilerini_yukle(self):
        def islem():
            try:
                res = requests.get(f"{FIREBASE_URL}/nahogram/kullanicilar/{self.aktif_kullanici}.json", timeout=10,
                                   verify=certifi.where())
                if res.status_code == 200 and res.json():
                    kullanici = res.json()
    ...
    def profil_guncelle(self, bio_yazisi, hedef_yazisi):
        ...
        def islem():
            try:
                requests.patch(f"{FIREBASE_URL}/nahogram/kullanicilar/{self.aktif_kullanici}.json",
                               json={"bio": bio_yazisi, "hedef": hedef_yazisi}, timeout=10, verify=certifi.where())
    ...
    def mesaj_listesini_olustur(self):
        if self.is_fetching_mesajlar: return
        self.is_fetching_mesajlar = True

        def islem():
            try:
                ben = requests.get(f"{FIREBASE_URL}/nahogram/kullanicilar/{self.aktif_kullanici}.json", timeout=10,
                                   verify=certifi.where()).json()
                if not ben:
                    self.is_fetching_mesajlar = False
                    return
                ...
                ham_kullanicilar = requests.get(f"{FIREBASE_URL}/nahogram/kullanicilar.json", timeout=15,
                                                verify=certifi.where()).json()
                kullanicilar = {}
                if isinstance(ham_kullanicilar, dict):
                    kullanicilar = ham_kullanicilar
    ...
    def sohbet_yenile(self, dt):
        ...
        def islem():
            try:
                res = requests.get(f"{FIREBASE_URL}/nahogram/sohbetler/{oda_id}.json", timeout=5,
                                   verify=certifi.where())
                if res.status_code == 200 and res.json():
                    gelen_veri = res.json()
                    mesaj_listesi = []
    ...
    def mesaj_gonder(self, mesaj):
        ...
        def islem():
            try:
                oda_id = "_".join(sorted([self.aktif_kullanici, self.aktif_sohbet_hedefi]))
                res = requests.get(f"{FIREBASE_URL}/nahogram/sohbetler/{oda_id}.json", timeout=10,
                                   verify=certifi.where())
                mevcut_mesajlar = []
                if res.status_code == 200 and res.json():
                    gelen = res.json()
                ...
                requests.put(f"{FIREBASE_URL}/nahogram/sohbetler/{oda_id}.json", json=mevcut_mesajlar, timeout=10,
                             verify=certifi.where())
            except:
                pass
